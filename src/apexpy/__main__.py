...
def main():
    ...
    parser = argparse.ArgumentParser(description='Converts between geodetic, modified apex, quasi-dipole and MLT',
                                     prog='apexpy')

    parser.add_argument('source', metavar='SOURCE', choices=['geo', 'apex', 'qd', 'mlt'], help='Convert from {geo, apex, qd, mlt}')
    ...
    parser.add_argument('--height', dest='height', default=0, metavar='HEIGHT', help='height for conversion')
    parser.add_argument('--refh', dest='refh', metavar='REFH', type=float,
                        default=0, help='reference height for modified apex coordinates')

    parser.add_argument('-i', '--input', dest='file_in', metavar='FILE_IN', type=argparse.FileType('r'),
                        default=STDIN, help='input file (stdin if none specified)')
    parser.add_argument('-o', '--output', dest='file_out', metavar='FILE_OUT', type=argparse.FileType('wb'),
